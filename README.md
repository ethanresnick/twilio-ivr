...
* `urlFor` (**required**): an object that helps the library generate urls.
  * `host` (**required**): a function that must return the [host](https://www.sistrix.com/ask-sistrix/onpage-optimisation/what-is-the-difference-between-a-url-domain-subdomain-hostname-etc/#The_structure_and_components_of_a_URL) that should be used by default in absolute urls generated by the library's `urlFor` function. Most likely, your function should just return a constant string. In case you need to do something more sophisticated, this function is passed express' request object. Note: be careful here about using the HTTP `Host` header to generate the return value, as that creates a [security risk](https://expressionengine.com/blog/http-host-and-server-name-security-issues).
  * `scheme` (optional): a function that returns the scheme to use by defualt in absolute urls generated by the library's `urlFor` function. If this is not provided, `req.protocol` will be used.
