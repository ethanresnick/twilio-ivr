...
describe("the hold music endpoint", () => {
    ...
    let holdMusicUrlState = util_1.stateRenderingUrlFor("/hold-music", "/get-hold-music-endpoint");
    let agent = request(_1.default([holdMusicUrlState], util_1.filesConfig(baseStaticConfig)));
    let mountPathAgent = request(_1.default([holdMusicUrlState], util_1.filesConfig(mountedStaticConfig)));
    describe("retrieval", () => {
    ...
    describe("urlFor", () => {
        ...
        it("should still work if the holdMusicEndpoint starts with the same string as the static mount path", () => {
            ...
            let holdMusicUrlState = util_1.stateRenderingUrlFor("/public-music", "/get-hold-music-endpoint");
            let agent = request(_1.default([holdMusicUrlState], util_1.filesConfig(baseStaticConfig)));
            return agent
                .post("/get-hold-music-endpoint")
